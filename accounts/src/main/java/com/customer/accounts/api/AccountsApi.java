...
public class AccountsApi {
  ...
  public ResponseEntity<?> findCustomerAccountByType(@RequestHeader(required = true) String customerId,
      @RequestHeader(required = true) String accountType){

    return new ResponseEntity<>(accountsService.customerAccountByType(customerId, accountType), HttpStatus.OK);
  }

  @PostMapping(path = "/customer", produces = "application/json")
  public ResponseEntity<?> createCustomerAccount(@RequestBody(required = true) Customer customer,
                                                 @RequestHeader(required = true) String accountType){

    return new ResponseEntity<>(accountsService.createCustomerAccount(customer, accountType), HttpStatus.OK);
  }
